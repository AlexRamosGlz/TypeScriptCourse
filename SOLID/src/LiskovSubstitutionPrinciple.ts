...
// TODO make an example of the 3rd rule

type OnlyPositiveNumber = {
  a: number
}

/**
 * 4 A subclass should't strengthen pre-conditions
 *
 */
class AnotherCalculator extends Calculator {
  // BAD
  // our base method declares both parameters to be numbers NOT caring if the are negative or not.
  // But we override the method and now it requires that the "a" parameter be completly positive values, this is a pre-condition.
  // the client code, which is used to work fine when passing negative numbers into the method, now breaks if it starts working with an object of this
  // subclass
  // public sum(a: OnlyPositiveNumber, b: number): number {
  //   if (a.a < 0) throw { error: "Negative value, throwing exception..." };
